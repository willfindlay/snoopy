...
class TestSyscallFormat(unittest.TestCase):
    def test_custom_syscall(self):
        FORMAT[500] = SyscallFormat((ARG_INT, ARG_STR, ARG_PTR, ARG_INT, ARG_INT, ("flag_1","flag_2","flag_3","flag_4")), ARG_INT)
    ...
    def test_flags_function(self):
        ...
        s = sf.flags(("flag_1","flag_2"), 2)
        self.assertEqual(s, "flag_2")

        s = sf.flags(("flag_1","flag_2"), 1)
        self.assertEqual(s, "flag_1")

        s = sf.flags(("flag_1","flag_2"), 0)
        self.assertEqual(s, "")

        s = sf.flags(("flag_1","flag_2","flag_3"), 3)
        self.assertEqual(s, "flag_1|flag_2")

        s = sf.flags(("a","b","c","d","e","f","g"), 0b1001100)
